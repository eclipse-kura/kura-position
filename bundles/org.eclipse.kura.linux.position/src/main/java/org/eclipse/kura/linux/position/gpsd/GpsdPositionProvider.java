...
public class GpsdPositionProvider implements PositionProvider, IObjectListener {
    ...
    private final AtomicReference<GpsdInternalState> defaultInternalStateReference = new AtomicReference<>(new GpsdInternalState());
    ...
    public void stop() {
        ...
        this.internalStateReferences.clear();
    ...
    private AtomicReference<GpsdInternalState> getInternalStateReference() {
        return this.internalStateReferences.entrySet().stream()
                ...
                .orElse(this.defaultInternalStateReference);
    }

    private boolean hasValidPosition() {
        return this.internalStateReferences.entrySet().stream().anyMatch(entry -> {
            boolean isValid = entry.getValue().get().isValid();
            if (isValid) {
                logger.debug(entry.getKey() + " has a valid position.");
            }
            return isValid;
        });
    }
    ...
    public LocalDateTime getDateTime() {
        return LocalDateTime.ofEpochSecond(getInternalStateReference().get().getTimestamp().longValue(), 0, ZoneOffset.UTC);
    }
    ...
    public boolean isLocked() {
        ...
        ENMEAMode enmeaMode = getInternalStateReference().get().getMode();
    ...
    public synchronized void handleTPV(TPVObject tpv) {
        ...
        final String device = tpv.getDevice();
        if (device == null) {
            logger.debug("TPV object without device field: {}", tpv);
            return;
        }

        GpsdInternalState oldInternalState;
        if (this.internalStateReferences.get(device) == null) {
            oldInternalState = new GpsdInternalState();
        } else {
            oldInternalState = this.internalStateReferences.get(device).get();
        }
        ...
        boolean isLastPositionValid = hasValidPosition();
        this.internalStateReferences.put(device, new AtomicReference<>(newState));
        boolean isNewPositionValid = hasValidPosition();
    ...
    private synchronized void checkFixValidity(final long validityIntervalNanos) {
        final GpsdInternalState currentState = getInternalStateReference().get();
